...
from judo_data.config import ACTIONS, BASE_URL, REQUEST_DELAY, REQUEST_TIMEOUT
...
class JudoFetcher:
    ...
    async def fetch_all_athletes(self, athlete_ids: list[int]) -> list[dict]:
        """Busca dados de múltiplos atletas com rate limiting.

        Faz uma chamada por atleta, respeitando o delay configurado
        entre requests consecutivos para evitar bloqueio pela API.
        ...
        for i, athlete_id in enumerate(athlete_ids):
            ...
            data = await self.fetch_athlete(athlete_id)
            results.append(data)
        return results
